...
#Get current epoch
current_epoch=$(docker logs --tail 50 monad-bft-1 | grep -o 'epoch: [0-9]*' | tail -n 1 | awk '{print $2}')

#Get Current round
current_round=$(docker logs --tail 50 monad-bft-1 | grep -o 'round: [0-9]*' | tail -n 1 | awk '{print $2}')
