...
used_with_unit=$(/usr/local/bin/monad-mpt --storage /dev/$TARGET_DRIVE | grep -A 1 'Capacity' | tail -n 1 | awk '{print $3, $4}' | tr -d '\r')
capacity_with_unit=$(/usr/local/bin/monad-mpt --storage /dev/$TARGET_DRIVE | grep -A 1 'Capacity' | tail -n 1 | awk '{print $1, $2}' | tr -d '\r')
...
current_epoch=$(tail -500 /var/log/syslog | grep -o 'epoch: [0-9]*' | tail -n 1 | awk '{print $2}')

# Get current round from syslog
current_round=$(tail -500 /var/log/syslog | grep -o 'round: [0-9]*' | tail -n 1 | awk '{print $2}')
